...
public class CondCP_hasPlaced extends Condition {

    static {
        Skript.registerCondition(CondCP_hasPlaced.class, "[core[ ]protect] %string% (1¦[have|has|had]|1¦[hav|has|had](n't| not)) placed %block% before %timespan% [ago] offset [by] %timespan%");
    }
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "[core[ ]protect] %string% (1¦[have|has|had]|1¦[hav|has|had](n't| not)) placed %block% before %timespan% [ago] offset [by] %timespan%";
    }
    ...
    public boolean check(Event event) {
        if (Creatio.getInstance().hookedCP()) {
            ...
            if (enable) return this.isNegated();
