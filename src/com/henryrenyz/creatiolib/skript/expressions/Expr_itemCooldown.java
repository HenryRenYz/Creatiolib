...
public class Expr_itemCooldown extends SimpleExpression<Number> {
    ...
    public void change(Event event, Object[] delta, Changer.ChangeMode mode){
        ...
        if (mode == Changer.ChangeMode.SET) {
            if (clientside) {
                Object packet = Packets.PlayOutSetCooldown(item.getSingle(event).getType(), ((Number) delta[0]).intValue());
                Packets.sendPacket(new Player[]{player.getSingle(event)},packet);
                return;
            } else {
